...
:: 4. Iniciar llama-server (Modo Roteador)
echo [2/3] Iniciando llama-server em modo roteador...
start "llama-server" /min "%LLAMA_EXE%" --models-dir "%MODELS_DIR%" --models-max 1 --port 8080 --tools all --jinja --host 0.0.0.0 -c 32768 --flash-attn on --no-mmap --cache-type-k q8_0 --cache-type-v q8_0 --parallel 1
